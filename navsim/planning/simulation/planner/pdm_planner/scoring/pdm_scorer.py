...
class PDMScorer:
    ...
    def _calculate_lane_keeping(self) -> None:
        ...
        若轨迹在非交叉口区域连续超横向偏离阈值达指定时长，则判定失败
        """
        # Initialize lane-keeping scores to 1.0
        lane_keeping_scores = np.ones(self._num_proposals, dtype=np.float64)
        lateral_deviation_limit = self._config.lane_keeping_deviation_limit     # 配置中获取最大横线偏离距离
        ...
        centerline = self._centerline.linestring    # 加载车道中心线

        # 遍历所有候选轨迹（每个proposal代表一条候选轨迹）
        for proposal_idx in range(self._num_proposals):
            consecutive_exceeds = 0 
            # 遍历轨迹的每个时间采样点（包含起始点，故+1）
            for time_idx in range(self.proposal_sampling.num_poses + 1):

                # 获取当前时刻车辆中心点坐标（BBCoordsIndex.CENTER对应坐标索引）
                ...
                lateral_deviation = ego_position.distance(centerline)  # 计算横向偏离距离（点到车道中心线的最短距离）
