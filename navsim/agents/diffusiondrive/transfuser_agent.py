...
class TransfuserAgent(AbstractAgent):
    ...
    def __init__(
        ...
        # 轨迹采样总时长4s，采样间隔0.5s，总的轨迹点数为9（包含起始点）
        #-----------------------------------
    ...
    def get_sensor_config(self) -> SensorConfig:
        """Inherited, see superclass."""
        #-------------------------------------
        #return SensorConfig.build_all_sensors(include=[3])  #加载所有传感器的当前帧
        history_steps = [3]
        return SensorConfig(
    ...
    # 特征以字典的形式提供，其中包含feature_builders的所有特征。
    # 前向传递必须输出一个 Dict，其中 1 个条目必须是 “trajectory” 并包含一个代表未来轨迹的张量，即形状为 [B， T， 3]
    # target最早是从这里传入的
    ...
    # 输入targers和predictions，返回一个字典，包含所有损失项
    def compute_loss(
        self,
