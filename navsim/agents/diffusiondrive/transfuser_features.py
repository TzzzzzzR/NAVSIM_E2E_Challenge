...
class TransfuserFeatureBuilder(AbstractFeatureBuilder):
    ...
    def compute_features(self, agent_input: AgentInput) -> Dict[str, torch.Tensor]:
        ...
        features["status_feature"] = torch.concatenate(
            [
                torch.tensor(agent_input.ego_statuses[-1].driving_command, dtype=torch.float32),    # status_feature中包含了导航指令，自车速度和加速度
    ...
    def _get_camera_feature(self, agent_input: AgentInput) -> torch.Tensor:
        ...
        cameras = agent_input.cameras[-1]   # 取最新摄像头数据

        # Crop to ensure 4:1 aspect ratio，摄像头裁剪
        l0 = cameras.cam_l0.image[28:-28, 416:-416]
        f0 = cameras.cam_f0.image[28:-28]
        r0 = cameras.cam_r0.image[28:-28, 416:-416]

        # stitch l0, f0, r0 images，画面拼接
        stitched_image = np.concatenate([l0, f0, r0], axis=1)
        resized_image = cv2.resize(stitched_image, (1024, 256))
        # resized_image = cv2.resize(stitched_image, (2048, 512)) 尺寸标准化

        # 转化为[3,256,1024]的Tensor，只进行预处理和格式转换，没进行特征提取
...
class TransfuserTargetBuilder(AbstractTargetBuilder):
    ...
    def compute_targets(self, scene: Scene) -> Dict[str, torch.Tensor]:
        ...
        frame_idx = scene.scene_metadata.num_history_frames - 1 #当前帧索引frame_idx
        annotations = scene.frames[frame_idx].annotations       # 获取当前帧标注信息
        ...
        #---------
        agent_future_traj = self._compute_agent_future_traj(
            current_frame_idx=frame_idx,
            scene=scene,
            num_future_steps=self._trajectory_sampling.num_poses  # 未来轨迹GT与ego轨迹步长一致
        )
        #-----------

        return {    # 当前帧的GT信息
            "trajectory": trajectory,       # 自车未来轨迹
            "agent_states": agent_states,   # 所有agent的2D bbox信息
            "agent_labels": agent_labels,   # agent掩码，图中哪些位置是agent
            "bev_semantic_map": bev_semantic_map,

            "agent_future_traj": agent_future_traj # 新增agent未来轨迹
        }
    
    #----------------------------
    # 计算所有agent的未来轨迹（用于轨迹预测的GT）
    def _compute_agent_future_traj(
            self,
        ...
        """
        提取每个agent的未来轨迹, 基于track_token实现跨帧关联
        :param current_frame_idx: 当前帧索引
        :param scene: 场景对象
        :param num_future_steps: 需要提取的未来轨迹步长
        :return: 未来轨迹张量 [max_agents, num_future_steps, 2]->[agent数量,每个agent的未来轨迹步长,2D坐标]
        """
        # 定义LiDAR范围过滤函数（与agent_states处理一致）
        ...
        max_agents = self._config.num_bounding_boxes    # 配置定义的最大允许处理的agent数量（同检测任务）
        future_traj = torch.zeros((max_agents, num_future_steps, 2), dtype=torch.float32)  # 初始化输出尺寸

        current_frame = scene.frames[current_frame_idx] 
        current_agents = {} # 用于存储当前帧的agent信息（track_token和位置）

        # 遍历当前帧的所有agent，并记录它们的token和坐标
        # boxexs：每个agent的Bounding Box信息。name：agent类型（如"vehicle"、"pedestrian"）。token：agent的唯一标识符（跨帧不变）
        for box, name, token in zip(current_frame.annotations.boxes, current_frame.annotations.names, current_frame.annotations.track_tokens):
            if name == "vehicle" :   
                x, y = box[BoundingBoxIndex.X], box[BoundingBoxIndex.Y]
                if _xy_in_lidar(x, y, self._config):  # 限制agent当前位置范围在LiDAR的范围内
                    current_agents[token] = (x, y)

        # 遍历未来帧，每次for执行其中一个未来帧
        for step in range(1, num_future_steps + 1):
            if current_frame_idx + step >= len(scene.frames):   # 逐帧访问未来时间步的数据，直到场景结束
                break
            future_frame = scene.frames[current_frame_idx + step]

            for token, box, name in zip(future_frame.annotations.track_tokens, future_frame.annotations.boxes, future_frame.annotations.names):
                if name != "vehicle" or token not in current_agents:   # 跳过未来帧非车辆类型的agent，或者当前帧没有的agent
                    continue
                # 检查这个未来帧坐标是否在LiDAR范围内，超出范围则跳过，不填入
                if not _xy_in_lidar(box[BoundingBoxIndex.X], box[BoundingBoxIndex.Y], self._config):
                    continue 
                # 依次填入未来轨迹
                idx = list(current_agents.keys()).index(token)  # 获取当前agent在current_agents中的的索引（用于将轨迹填入正确的位置）
                if idx < max_agents:
                    future_traj[idx, step-1, 0] = box[BoundingBoxIndex.X]
                    future_traj[idx, step-1, 1] = box[BoundingBoxIndex.Y]
        return future_traj
    #--------------------

    # 计算所有agent的2D bbox信息(用于agent检测的GT)
    ...
    def _compute_map_polygon_mask(
        ...
        """
        # 查询自车周围指定半径（bev_radius）内的多边形地图元素（）
        ...
        # 将多边形从全局坐标系转换到自车局部坐标系。将局部坐标（米为单位）转换为BEV栅格的像素坐标
        for layer in layers:
